		...
		foreach ($files['name'] as $key => $value) {
				...
				if ($files['name'][$key]) {
					...
					if ($dateityp[2] != 1 && $dateityp[2] != 2 && $dateityp[2] != 3) {			
						...
						echo "Don't manipulate the upload"; die;
	...
	//	$uploads = wp_upload_dir();
		...
		if ($files['size'] > 0) {
		  ...
		  $dateigroesse = $files['size'][$key];
			...
			// Notwendig: wird in convert-several-pics so verabeitet
			...
			else if($post_type=="image/png"){$post_type="png";$imageType="jpg";}
			else if($post_type=="image/gif"){$post_type="gif";$imageType="jpg";}
			else{	
				$doNotProcess=1;
			}
		...
		// Später für Inform Image wichtig	
