...
//echo "GaleryID: $galeryID<br/>";
	...
	if($picsSQL1==NULL){
		echo "No gallery under this id";		
			...
			echo "<div>";
			echo '<form action="'.$path.'" method="post" id="cg_upload_form" enctype="multipart/form-data" novalidate >';
			...
			*/
			...
			//print_r($getUploadForm );
					...
					if (@$value->Field_Type=='text-f'){
						...
						echo "<input type='text' placeholder='$content' class='cg_input_text_class' id='cg_upload_form_field$id' value='' name='form_input[]'>";// Content Feld, länge wird überprüft
						...
						echo "<input type='hidden' class='cg_form_required' value='$checkIfNeed'>";// Prüfen ob Pflichteingabe
						echo "<p class='cg_input_error'></p>";// Fehlermeldung erscheint hier
						echo "</div>";
					...
					if (@$value->Field_Type=='email-f'){
						...
						if(is_user_logged_in()==false){
							...
							echo "<input type='hidden' name='form_input[]'  value='$Field_Order'>";// Feldreihenfolge wird mitgegeben
							echo "<input type='text' placeholder='$content' value='' class='cg_input_email_class' id='cg_upload_form_field$id' name='form_input[]'>";// Content Feld, länge wird überprüft
							echo "<input type='hidden' class='cg_form_required' value='$checkIfNeed'>";// Prüfen ob Pflichteingabe
							echo "<p class='cg_input_error'></p>";// Fehlermeldung erscheint hier
							echo "</div>";
					...
					if (@$value->Field_Type=='comment-f'){
						...
						echo "<input type='hidden' name='form_input[]'  value='$Field_Order'>";// Feldreihenfolge wird mitgegeben
						echo "<textarea maxlength='$maxsize' class='cg_textarea_class' id='cg_upload_form_field$id' placeholder='$content' name='form_input[]'  rows='10' ></textarea>";// Content Feld, länge wird überprüft
						echo "<input type='hidden' class='minsize' value='$minsize'>"; // Prüfen minimale Anzahl zeichen
						echo "<input type='hidden' class='maxsize' value='$maxsize'>"; // Prüfen maximale Anzahl zeichen
						echo "<input type='hidden' class='cg_form_required' value='$checkIfNeed'>";// Prüfen ob Pflichteingabe
						echo "<p class='cg_input_error'></p>";// Fehlermeldung erscheint hier
			...
			echo "<div>";
			echo '<input type="submit" name="cg_form_submit" id="cg_users_upload_submit">';
			echo "</div>";
			//echo "</div>";
			echo '</form>';
			echo "</div>";
			...
			$formOutput = ob_get_clean();
				...
				if(@$proOptions->RegUserUploadOnly==1 && is_user_logged_in()==false){
					echo @$proOptions->RegUserUploadOnlyText;		
				}
				else{
