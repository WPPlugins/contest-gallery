...
elseif($type=="gallery"){
	...
	$getOptions = $wpdb->get_row( "SELECT CheckLogin, AllowRating, ShowOnlyUsersVotes, IpBlock, VotesPerUser, HideUntilVote FROM $tablenameOptions WHERE id = '$galeryID'" );
	...
	if ($CheckLogin == 1)
	{
		...
		"
			...
			FROM $tablenameIP 
			WHERE pid = %d and GalleryID = %d and WpUserId = %s
		", 
		$pictureID,$galeryID,get_current_user_id()
		) );
	}
	else
	{
...
<?php
...
else if ((@$countVotesOfUserPerGallery >= @$VotesPerUser) && (@$VotesPerUser!=0 || @$VotesPerUser!='')){
			...
			if($rateValue<=5 and $rateValue>=1){					
					...
					if(@$countR>0){@$countR="<b>&nbsp;($countR)</b>";}
					  ...
					  echo "<div style='display:inline-block;float:left;$starDiv;vertical-align: middle;'><img src='$star4img' class='$star4' style='float:left;cursor:pointer;$starDiv;' alt='4' id='cg_rate_star$pictureID'></div>";
					  echo "<div style='display:inline-block;float:left;$starDiv;vertical-align: middle;'><img src='$star5img' class='$star5' style='float:left;cursor:pointer;$starDiv;' alt='5' id='cg_rate_star$pictureID'></div>";
					  echo "<div style='display:inline-block;float:left;$fontDivHeight;vertical-align: middle;color:".$fontColor.";' class='$classRatingCount'>$countR</div>";
			...
			if($rateValue==6){
				...
				if(@$countS>0){$star6 = "cg_slider_star1"; $star1img = $starON;@$countS="<b>&nbsp;($countS)</b>";}
				else{$star6 = "cg_slider_star"; $star1img = $starOFF;@$countS="";}
				
				 echo "<div style='display:inline-block;float:left;$starDiv;vertical-align: middle;'><img src='$star1img' class='$star6' style='float:left;cursor:pointer;$starDiv;' alt='6' id='cg_rate_star$pictureID'></div>";
				 echo "<div style='display:inline-block;float:left;$fontDivHeight;vertical-align: middle;color:".$fontColor.";' class='$classRatingCount'>$countS</div>";	
				 echo "<input type='hidden' id='cg_AlreadyRated' value='".$alreadyVotedText."'>";	
			}
