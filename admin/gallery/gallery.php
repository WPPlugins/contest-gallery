	...
	// Link zum Wordpress User in WP Management ---- ENDE
		...
		if ($selectFormInput == true) {
					...
					foreach($selectContentFieldArray as $key => $formvalue){
							...
							echo "<input type='text' value='$getEntries1' name='content[]'  style='width: 500px;' maxlength='100'>";
	...
	//echo "<br>Informed: $Informed<br>";
		...
		if($Active==1){
				...
				if ($Inform == 1) {
					...
					$InformedId = ($Informed==1) ? "" : "<input type='hidden' name='informId[]' value=\"$id\" class='image-delete' >";
