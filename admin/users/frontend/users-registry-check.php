...
if($_POST['cg_check']==wp_create_nonce("check") && @$_POST['cg_Fields']==true){
		...
		// Einfügen von Werten mit Kennzeichnung durch Activation Key zur späteren Wiederfindung --- ENDE
			...
			@$TextEmailConfirmation = html_entity_decode(stripslashes(@$proOptions->TextEmailConfirmation));
			@$ForwardAfterRegText = html_entity_decode(stripslashes(@$proOptions->ForwardAfterRegText));
			...
			else{@$TextEmailConfirmation = str_replace(@$posUrl, get_permalink()."?cgkey=$activation_key#cg_activation", @$TextEmailConfirmation);}			
