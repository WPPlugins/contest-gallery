...
if(@$_GET["cgkey"]){
	...
	//echo "222";
			...
			// Zwei Tests notwendig um zu prüfen ob User schon registriert ist
					...
					foreach($userAccountEntries as $key => $value){
						...
						if($checkUserViaMailName){
							...
							//	die;
								...
								// Den Einträgen wird die neue WP ID hinzugefügt
								$wpdb->update(
...
foreach($selectUserForm as $key => $value) {
				...
				echo "<label for='cg_registry_form_field".$value->id."' >".$value->Field_Name." $required</label>";			
					...
					// Prüfen ob check-agreement-feld ist ansonsten Text oder, Comment Felder anzeigen
					if (@$value->Field_Type=='user-check-agreement-field'){
						...
						echo @$cgContentField;
						echo "<input type='hidden' class='cg_Min_Char' value='".$value->Min_Char."'>"; // Prüfen minimale Anzahl zeichen
						echo "<input type='hidden' class='cg_Max_Char' value='".$value->Max_Char."'>"; // Prüfen maximale Anzahl zeichen
						echo "<input type='hidden' class='cg_form_required' value='".$value->Required."'>";// Prüfen ob Pflichteingabe
					}
...
echo "<div>";
echo '<input type="submit" name="cg_registry_submit" id="cg_users_registry_check">';
echo "</div>";
echo '</form>';
echo "</div>";
echo "</div>";
